...
- **ROI 对游戏帧率的影响**：上一轮没测出（且当轮参考漂移 3.3 fps、第四段被截断，判读不了）。所以默认开启的依据是**已证实的等效性 + 采集线程占用减半（28% → 14%）+ PCIe 流量 422 → ~18 MB/s**，**不是**"帧率更高"。想要帧率结论就得重跑一次干净的 A/B——已在 **§32.8** 用一次同期交替 A/B 补上：ROI 领先约 1%、尾部纹丝不动、且顺序与处理混杂 ⟹ 仍不支持"帧率更高"。注意 §32.8 同时查出该脚本的 `Fps` 列约为真实帧率的 2 倍。
- DPI 100%/150%、设备重建、长时间运行。
