...
## 9.6 阈值定不下来在哪，以及为什么不需要出去采集

用**现有数据**就能把两端量出来，不必逐地区截图：

| 证据 | 数值 |
| --- | --- |
| 真实层内帧 ×2（下层金库 1 楼） | 自绘票 **7 vs 1** 和 **4 vs 2** |
| 真实地表帧 ×2（别处） | unknown |
| 大世界地表位置 ×702 | **任何阈值 1~30 都不采纳**（0%）|
| 下层金库地表位置 ×122 | 阈值 8 → 0%；阈值 6 → 15.6%；阈值 4 → 19.7% |

于是两端的可行区间是：

- **要让两张真实层内帧过**：阈值必须 ≤ **4**（7≥4 ✓；4≥4 且 4≥2×2 ✓）。
- **要让下层金库地表侧保持 0**：阈值必须 ≥ **8**。

**冲突无法用一个数字解决，而冲突只出现在一类楼层上**：自绘美术很薄的层。下层金库 1 楼的
"真·自绘"只占 4.5%（其余是抄地表的），所以它的自绘票在层内是 4~7、在地表也能到 6~7 ——
**对这一类楼层，自绘票本身分不开"在层里"和"在层外"**。

三条可选，我倾向第一条：

1. **保持 8**，并把这类薄层交给既有的 `sharedGround`（"两边都显示"）——那本来就是为"影像分不开"
   准备的答案，而且下层金库门前广场的既定结论就是"两边都显示"。代价：薄层在**洞内**的标记过滤
   不生效（看到所有层的标记，而不是只看到本层）。
2. **降到 4**：真实层内帧恢复正常，大世界仍然 0%（实测）；代价是下层金库地表侧 19.7%、黯原 9.0%、
   隐海 5.3% 会重新藏地表标记——也就是**把这次修的这类 bug 重新引进子世界**。
3. **按层的自绘描述子数量自适应**（例如 <200 个的层用 4，其余用 8）：把代价限制在薄层上，但
   下层金库那 19.7% 依然存在，因为那就是薄层本身。

**不需要出去采集的标定路径**：本分支已经把 `ownWinner / ownRunnerUp / ownVotes` 写进了
`layered-floor` 诊断行。也就是说**下一位玩家提交的日志里就带着真实层内自绘票**——收到日志时
把它喂给 `IMaoLayeredFloorProbe --batch` 即可，不需要专门跑图。在这之前保持 8 是安全的一侧
（不会新增"标记消失"），薄层的代价是"过滤不生效"，比反过来轻。
