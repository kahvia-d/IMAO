...
规模（`map-regions/layers/layers.manifest.json`）：**57 个分层地图 / 90 个楼层 / 161 张分层瓦片**。

| | 分层地图 | 楼层 | 分层瓦片 |
| --- | ---: | ---: | ---: |
| World（frame 8：今州 4 / 梦州 6 / 拉古那 8 / 七丘 7 / 冰原 5 / 黑海岸 0） | 30 | 44 | 77 |
| 拉海洛（906） | 14 | 23 | 49 |
| 下层金库（902） | 4 | 10 | 15 |
| 黯原（909） | 5 | 7 | 9 |
| 泰缇斯之底（900） | 3 | 5 | 7 |
| 隐海试验场（905） | 1 | 1 | 4 |
| 阿维纽林（903）/ 时隙废都（910） | 0 | 0 | 0 |

### 归属规则（`scripts/LayeredRegionAttribution.ps1`，两个脚本共用）

- **frame ≠ 8**：整个 frame 属于该地区（小世界，一个 frame 一个地区）。
- **frame = 8**：取"**二级地区锚点**中最接近该分层入口（FCRK）的那个"，其 `mapState` 决定地区。
  只允许 frame 8 的地区参与竞争——否则 **拉海洛（frame 906、mapState 5、countryId 4）会抢走
  天槎空间站**。
- 没有入口标记的分层（35 个里有一部分）用**自身瓦片中心**兜底。

旧规则（"任意层级锚点 + mapState 相等")实测是错的：大量**三级节点 mapState 为空**，于是
石龙寝（七丘）和天槎空间站（冰原）被判给 **mapState 同样为空的黑海岸**，而七丘/冰原被告知
"没有分层地图"。修正后 **57 个分层地图、90 个楼层，全部归属且无重复**。

两个踩过的 PowerShell 坑（都写进脚本注释了）：`@($a * k + o, $b * k + o)` 里 **`,` 的优先级
高于 `+`**；以及调用 PowerShell 脚本后读 `$LASTEXITCODE` 在 `StrictMode` 下会直接报错。

### 运行时（90 层同台竞技）

- 冷启动多选题：**90 层**一起比对 ✓。实测：两帧实机洞穴小地图仍然以 **22 : 7 / 22 : 5**
  选出正确楼层；**9 张地表参考全部保持 unknown（0 误判）**——地表帧的票数被"共享的压暗底片"
  摊平（出现 `lowervault 20 vs 20` 这种并列），永远拉不开 2 倍。
- 每层描述子上限 **600**（`kFloorDescriptorCap`）：全量 22:7 / 22:5、600 时 26:12 / 20:7，
  而 300 会直接判错（22:14、以及 9:8 的错答案）。
- 每帧分类改为**传指针**（`Classify(query, std::vector<const FloorEntry*>&)`）：90 层全量时
  按值拷贝是每次 **27 MB** 描述子。

### 共用地表判定（2026-09-23 傍晚）

问题：站在**下层金库门前广场**（贵金属与艺术品藏区 1 楼覆盖到的地方）时，工具认为玩家在分层内，
把地表标记全藏了；而广场属于地表，玩家身边 3 单位处就有地表标记。

判定不是逐地区写死的，而是从瓦片本身算出来的：生成楼层索引时，把每张分层瓦片与**同坐标地表瓦片**
逐格比对（每格 4 个采样点，≥70% 的点颜色差 ≤30 即算"抄的地表"），结果作为 `shared` 网格随索引下发。
运行时看玩家 7×7 格邻域里 shared 占不透明格的比例，连续 3 帧 ≥0.5 就认定站在共用地面上。
全游戏 161 张分层瓦片、58,532 个不透明格统计：86.9% 明确是分层自绘（匹配 ≤30%）、8.5% 明确是共用，
中间只有 4.7%；邻域半径 3 实测广场 0.67–0.69、馆内大厅 0.16–0.34，阈值 0.5 落在空档里。

**两个必须一起看的前提**（都是这轮加进来的，否则会误伤）：

1. **只有紧邻地表的那一层（level = -1）才做这个比对** `LayeredFloors::AdjacentToSurface`。上层/下层
   是另一个地方，它们的美术哪怕抄了地表图也不是"地表地面"——拉海洛的**星炬学院系列整层 51%–84%
   的像素就是地表自己的画**（其余楼层最多 32%），比对会对着分层自己的地面报"共用"。上游在每张
   分层地图上都把紧邻地表的那层编号为 -1（洞穴的入口层、建筑的**地面层**：下层金库的 1 楼、
   星炬学院的**广场区**），所以这个判据不需要逐图调参。
2. **共用地面是"两边都算"**：站在共用地面上时不再清空楼层状态，而是**楼层照常显示自己的标记**，
   同时**地表标记恢复显示**（`Snapshot::sharedGround` + `MarkerRole::Normal`）。因为影像无法判断
   玩家指的是哪一层，藏掉任何一边都是猜；藏掉分层自己的标记更糟（星炬学院·广场区 10 个点里
   6 个、下层金库 0 个站在共用像素上）。

### 楼层上下方向（`LayeredFloors::AssignHeightRanks`）

上游的 `sort` 字段没用：它只是照着 level 顺序排（叩天关的"上层"和下层金库的"1楼"都排第一，物理上
正好相反），数据里也没有任何字段带高度。所以顺序只能从**名字**里读。而且**光有一个"方向"不够**：
虚妄摇篮的高度并不随 level 单调（二层最低、入口居中、一层最高），任何单一方向都表达不了。因此每层
带一个 `heightRank`（数字越大越高，仅在同一条分层地图内比较），运行时优先按 rank 比较，rank 缺失时
才退回"level × 方向"的老办法。

名字的读法，按优先级：

- **"…4楼"**：本作这类名字从地面往上编号（下层金库的 1 楼、声骸藏区的 1 楼都是地面层）。
- **"…二层"/"…地下一层"**：中文数字同理；带"地下"前缀的向下计数，所以地下一层在地上二层之上。
- **完全没有序号的命名**记在 `kRecordedOrders`（游戏事实，非数据事实，2026-09-23 实机确认）：
  - 星炬学院（地上多层建筑）：**广场区(1) < 教学区(2) < 运载区(3)**。
  - 虚妄摇篮：**二层(1) < 入口(2) < 一层(3)**——注意它的 level 是 -3 / -1 / -2，**不单调**，
    这也是必须有 rank 而不能只有方向的原因。
  - 拉海"日树"系列名字里的"一层/二层/三层"是从下往上数（一层最低），由中文数字规则直接得出。
...
`IMaoLayeredFloorProbe` 不带 `--map` 时会打印每个分层的 `heightDirection=` 以及每层的
`heightRank=` / `adjacent=`，方向不对一眼可见。
