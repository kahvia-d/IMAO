...
### 楼层上下方向（`LayeredFloors::LayerHeightDirection`）

上游的 `sort` 字段没用：它只是照着 level 顺序排（叩天关的"上层"和下层金库的"1楼"都排第一，物理上
正好相反），数据里也没有任何字段带高度。所以顺序只能从**名字**里读，而只读不会歧义的那部分：

- **"…4楼"**：本作这类名字从地面往上编号（下层金库的 1 楼、声骸藏区的 1 楼都是地面层）。
- **"区"字命名没有任何序号**，顺序是游戏事实而非数据事实，记在 `kZoneOrders` 里：星炬学院
  **广场区(1) < 教学区(2) < 运载区(3)**（2026-09-23 实机确认，该建筑是地上多层）。
- **中文"一层/二层"故意不读**：同一套名字在拉海洛"日树"是往上数、在黯原"虚妄摇篮"是往下数，
  名字里没有任何线索能区分——猜错一边就等于把另一边的 ^/v 标反。
...
`IMaoLayeredFloorProbe` 不带 `--map` 时每个分层会打印一行 `heightDirection=`，方向不对一眼可见。
