...
1. **整层自绘闸门** `LayeredFloors::ArtIsSurfaceCopy`：拉海洛的**星炬学院系列楼层是照着地表图画的**，
   整层 51%–84% 的像素就是地表自己的画（其余楼层最多 16%），比对会对**分层自己的地面**报"共用"，
   把玩家脚下的标记全藏掉。整层共用比例 ≥0.5 时直接停用该层的比对。
2. **共用地面是"两边都算"**：站在共用地面上时不再清空楼层状态，而是**楼层照常显示自己的标记**，
   同时**地表标记恢复显示**（`Snapshot::sharedGround` + `MarkerRole::Normal`）。因为影像无法判断
   玩家指的是哪一层，藏掉任何一边都是猜；藏掉分层自己的标记更糟（广场边 7 个点里就有 6 个在
   共用像素上）。
...
`IMaoLayeredFloorProbe --map x,y`（直接打印 `copied=` / `shared=` / `gated=`）。
