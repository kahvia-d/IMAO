...
1. **两个条件同时成立才做这个比对** `LayeredFloors::SharesSurfaceGround`：
   - **紧邻地表的那一层**（level = -1）：上游在每张分层地图上都把紧邻地表的那层编号为 -1
     （洞穴的入口层、建筑的**地面层**）。上层/下层是另一个地方，它们的美术哪怕抄了地表图也不是
     "地表地面"。
   - **该层美术不是照着地表画的**（`ArtIsSurfaceCopy`，整层抄图比例 < 0.5）：拉海洛的
     **星炬学院四个楼层是 58%–84% 照着地表图画的**（其余楼层最多 32%），比对会对**分层自己的
     地面**报"共用"。2026-09-23 晚实机反馈：站在星炬学院**广场区**（level -1，抄图 0.73）时
     日志出现 `layered-floor-shared shared=1 fraction=1.0 copiedFraction=0.73`，学院外的地表标记
     全被放了出来——那层的地面是学院自己的，不是地表的。下层金库广场（1 楼，抄图 0.27–0.32）
     两个条件都满足，所以那边的修复不受影响。
...
### 判定用"全场景同台"而不是"只比包含玩家的那几层"（2026-09-23 晚）

实机日志（黯原·虚妄摇篮，18:14–18:17）：玩家走出洞后状态仍挂着 `-2/52`，22 秒后才被收掉。
原因不在位置判据，而在**候选集**：

- 旧代码在"玩家位于某层足迹内"时，**只把包含玩家的楼层**送去比对，并用低门槛（4 票）+ 相对优势
  判为"认出来了"。只有一个候选时，它必然"赢"——于是洞外上方的一帧 4 票就变成了决定，
  把洞的状态一直按住。
- **帧的跨层分布本身就是"我在不在洞里"的证据**：地表帧会几乎同等地匹配该洞穴的每一层
  （实机：21/21/18；玩家自己那张地表截图：`-3/52=13, -1/52=11, -2/52=11` → unknown），
  而在洞里洞穴纹理会把正确层拉出 2 倍优势（叩天关 22 vs 7）。

所以现在：**整个场景的楼层一起比对**（门槛 4 票 + 2 倍优势），并且要求**胜出层必须是玩家位置
可能的所在层**（`Contains`）——足迹之外的胜出层是"玩家脚下的洞"，不是"玩家所在的洞"。
2 倍优势仍拉不开时（下层金库四个大理石厅 12/10/5/4）保留"并列也算"的通道，但加了两个条件：
胜出层必须包含玩家、票数 ≥10（`kNearTieMatches`）、且**与胜出层票数接近的层也必须是包含玩家的层**
（"唯一的候选赢了"不是证据；地表帧的并列恰恰横跨了玩家不在的层）。

诊断里新增 `winnerContained=`；`votes=[...]` 现在是全场景票表，地表帧的"平票"一眼可见。
