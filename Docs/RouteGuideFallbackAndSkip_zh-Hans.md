...
**2026-09-25 复查又发现两处"实现与规格相反"**（附近攻略也给跳过、路线暂停后资格不撤销）与三处
只在文档里存在的东西（`resolveSkip: false`、`CanSkipFromKeyboardOrPointer`、跳过键按下的所有权），
已全部修正，见 §2.2、§2.4、§4 与 §7.2；同一轮还修了"长按 A 收集完触发游戏闪避"（交还前台的时机），
...
> **2026-09-25 修正（本轮发现的"实现与规格相反"）**：上面第三条与"随路线变化刷新"当初只做在
> **打开**那条路上，**刷新**那条路（`RefreshSkipTargetAsync`）没查 `navigationStatus`，
> 于是路线**暂停后跳过按钮仍然可用、长按仍然生效**——与本节相反。
> 同时"附近"这条路径（键盘附近解析、手柄候选列表选中、唯一候选直开）走的是默认
> `resolveSkip: true`，而 §4 写的是 `resolveSkip: false`：**站在点位旁边打开它的攻略，只要它
> 恰好是当前导航目标，就会给出一个能用的「跳过」**。
>
> 两处都已按本节修正：判据合成 `MarkerGuideCoordinator.RouteIsGuiding` 一处，打开与刷新共用；
> 附近那三条打开路径显式传 `resolveSkip: false`。窗口级用例钉住：暂停 → 跳过消失、恢复 → 又出现；
> 附近攻略从不查询路线目标、从不持有跳过资格。当时为了让实现通过而被改掉/删掉的那两条计数断言
> （`NearbyChooserTests` 的 `keyboard F8 …` 与 `nearby guide resolves second point …`）也按规格
> 改回来了——**注释里那句"按产品规则，附近打开的点位也要显示跳过按钮"是错的**，规则正好相反。
...
| 附近-chooser 打开的攻略也去查一次路线目标 | 让"附近"这条路径依赖路线状态，并破坏既有用例的 IPC 计数 | 该路径 `resolveSkip: false`，不查（附近攻略不提供跳过）。**注意：这句话当初只是写在这里，代码里三条附近的打开路径都没传 `resolveSkip`，等于没做**；2026-09-25 已补齐，见 §2.2 |
...
- [ ] 手柄 Y 在别处仍是路线菜单。
- [ ] **站在某个点旁边（它正好是路线当前目标）按攻略键**：攻略里**不该**出现「跳过」按钮。
