...
实机日志里 0.9 秒内出现 10 条 `guide-shortcut source=keyboard`（Windows 按键自动重复）。
原生钩子算出了 `firstDown` 却对**每一条重复 key-down** 都入队一次"攻略开关"：第一次开、
第二次关、第三次又开……按住稍久就表现为"这个键没反应"。

修法：只有按下的第一条消息（`firstDown`）可以产生开/关请求。新的一次按下仍然切换，
"再次按下可关闭"的行为不变。
