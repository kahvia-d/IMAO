...
（§7.4 记录了这一版第一次上线时把 F8 弄坏的回归）。
**2026-09-25 复查修了一处真正的"实现与规格相反"**：路线**暂停后跳过资格不撤销**
（§2.2 第三条只在打开路径上做了，刷新路径没做）。另外三处只在文档里存在的东西
（`CanSkipFromKeyboardOrPointer`、跳过键按下的所有权、`resolveSkip: false`）也一并处理了，
见 §2.2、§2.4、§4 与 §7.2；同一轮还修了"长按 A 收集完触发游戏闪避"（交还前台的时机），
见 [攻略浮窗](MarkerGuideWindow_20260908.md) 的 2026-09-25 小节。
...
| 范围内为空、路线**指引中**、且有当前导航目标 | 打开该目标的攻略（`navigating` 与 `waitingForLocation` 都算指引中） |
| 范围内为空但路线**不在指引中**（已暂停/已结束） | 不打开攻略，提示「附近没有未完成点位，也没有正在导航的路线目标。」 |
| 范围内为空且核心没有当前目标 | 同上那一句 |
| 定位不可用、档案或筛选刚变过 | 只提示「当前位置暂不可用，请等小地图定位恢复后重试。」，**绝不**回退 |

> 2026-09-25 补充：回退**只在路线指引中才生效**（实机反馈：路线暂停时按攻略键仍会打开路线当前
> 目标的攻略，让人莫名其妙）。判定用 `RoutePlanningState.IsGuiding`（`navigating` /
> `waitingForLocation`）——后者也算指引中，因为进大地图会清掉玩家定位，而那条路线仍在指引，
> 这正是大地图上按攻略键要能用的原因。
...
- **路线处于指引中**（`navigating` / `waitingForLocation`）。原生在暂停/结束时**仍然返回当前目标**
  （`NavigationLocked()` 只在没有活动路线时才让 `selection` 为空），所以前两条都能通过，必须单独挡住；
- 判定来自原生答复，**客户端路线快照只用于显示，不作为授权**。

**判据是"展示的这个点是不是当前导航目标"，与"这份攻略是从哪个入口打开的"无关。**
因此「附近」入口（键盘附近解析、手柄候选列表选中、唯一候选直开）打开的攻略**同样要核对资格**：
玩家走到点位旁边才发现这个点不好收、打算下次再来，正是这条入口存在的理由，这时候就该能就地跳过。
代价是附近打开时会多一次 `markerGetRouteGuide`——这是有意为之，不是浪费。

> ⚠️ **§4 里那句"附近攻略不提供跳过（`resolveSkip: false`）"是过期设计**，2026-09-25 由用户确认
> 作废：他最初确实设计成"附近不给跳过"，后来改成"走到旁边才知道不好收，所以要给跳过"。
> 别再照 §4 那句话去改代码。`MarkerGuideCoordinator.ShowCurrentAsync` 的 `resolveSkip` 参数现在
> 没有调用方传 `false`，保留它只是为了把"这条路径要不要核对资格"写在调用点上。
>
> **同一次复查中真正需要修的是下面这条**（它与本节第三条有关，与附近路径无关）：
> 上面第三条与"随路线变化刷新"当初只做在**打开**那条路上，**刷新**那条路
> （`RefreshSkipTargetAsync`）没查 `navigationStatus`，而原生在暂停时仍然返回当前目标，
> 于是**路线暂停后跳过按钮仍然可用、长按仍然生效**——与本节相反。
> 已修正：判据合成 `MarkerGuideCoordinator.RouteIsGuiding` 一处，打开与刷新共用。
> 窗口级用例钉住：暂停 → 跳过消失、恢复 → 又出现。
>
> **记录一次走错的路（避免重犯）**：复查时把 §4 那句过期设计当成了现行规格，一度把附近三条路径
> 改成 `resolveSkip: false`，并把 `NearbyChooserTests` 里两条**本来就正确**的计数断言
> （"附近会查一次路线目标"）改成了 `== 0`。用户指出后已全部还原，并且现在有正向断言钉住
> 「附近攻略的当前目标要给出跳过」（`RouteControllerTests` 的
...
- **键鼠不需要先点一下攻略窗口：** 玩家在游戏里按 Z（完成当前点位）或长按 G 时，前台是游戏、
  ...
  **"独占"这句话现在是真的了**：按下与抬起由同一次按下的归属配对（`GuideKeyOwnership`），
  原来抬起时会重新问一遍"攻略还在不在"，于是按住期间攻略被关掉的那一次会把一个 key-up 漏给游戏。
  仍有一处**已知不做保证**：按住不放时 Windows 发的自动重复消息在旧写法里会被放行给游戏，
  而归属表只保证"第一次按下 + 它的抬起"成对；如果玩家把跳过键绑到游戏里有用的键，
  按住不放仍可能让游戏收到自动重复。默认的 G 键不在游戏的常用键位上。
...
- **键盘与手柄各自计时**：两条通道互不影响，任一通道满 600 毫秒只提交一次。
  （**鼠标不在这条通道上**：`d97d977` 起「跳过」按钮改成单击即提交，窗口里只剩
  `keyboardSkipGesture` 一个实例。`GuideSkipHoldGesture` 的类注释、`GuideSkipHoldGestureTests`
  里"键鼠两个通道各自一个实例"那段描述的是更早的设计，读它时不要以为按钮还要按住。）
...
| 撤销资格时用 `StackTrace` 现取调用方方法名 | 异步方法上取到的常常是 `MoveNext`／状态机类型：既付了取栈的代价，又拿不到有用的名字 | 调用方显式传 `reason`（`not-guiding` / `route-changed` / `identity-mismatch` / `guide-closed` …），日志里直接可读 |
...
| 附近-chooser 打开的攻略也去查一次路线目标 | ❌ **这一行的结论是过期的**：曾经认为"附近"不该查路线目标、不该提供跳过 | **现行设计：附近那条路径照样查一次并给跳过**——走到点位旁边才发现不好收、想下次再来，正是这条入口的理由（2026-09-25 用户确认）。见 §2.2 的说明。这一行保留是为了防止再照它改代码 |
| `GamepadInputUpdate` 不带动作类别 | 手柄 A 完成与 Y 跳过共用一条进度通道，跳过进度会画到完成条上 | 更新里带上 `HoldAction` |
| 跳过键按下时吃掉、抬起时**重新问一遍**"攻略还在不在" | 按下被攻略吃掉、按住期间攻略关掉（跳过成功/完成当前点/按 B）时，那个 key-up 会被漏给游戏——而游戏从没收到对应的 key-down，表现是游戏里某个键像卡住或被松开 | 所有权由**第一次按下**决定并记在 `GuideKeyOwnership` 里（`GuideHotkeyRouting.h`），抬起只看它，按下与抬起永远配对。自动重复也不改变归属 |
| 新增跳过键默认 G 之后，老配置文件里的 G 可能已经绑给别的功能 | 两份绑定撞车 → 整份配置被判定"损坏"→ **所有设置恢复默认**（只因我们加了一个按键） | 配置 schema 升到 5：老文件里这个字段不可能来自玩家的选择，撞车时只把**新功能**置为「未设置」，其余设置原样保留（`RuntimeConfigurationStore.Migrate`） |
...
| `IMao-WinUI/Services/MarkerGuideCoordinator.cs` | 仅 `guide-empty` 回退；跳过资格刷新与提交（**每条打开路径都核对，含"附近"**）；键鼠图片键（`markerGuidePictureKey`）转交 |
...
修法：`CanSkip` 只表示"这个点位是当前导航目标"，**不包含任何前台判定**；
键鼠通道的按下/松开由原生钩子按"攻略窗口可见 + 游戏或攻略在前台"筛选后转交（见 §7.4），
窗口自己不再查前台——它要再查一次，就会把"游戏在前台、玩家在游戏里按键"这种正常情况判掉。

> ⚠️ 本节原先还写了"键鼠通道另用 `CanSkipFromKeyboardOrPointer`（要求前台）"。
> **该成员从未存在过**（全仓无此符号），而且 §7.4 定下的规则正是"不需要窗口在前台"，
> 所以那句话是过时的，别再照它改代码。窗口级用例
> （`Tests/GuideWindowRuntime/GuideWindowTests.cs` 的 `the skip key reaches a visible guide while
> the game has the foreground`）断言的是"游戏在前台也能计时"，方向与那句话相反。
...
**这一版第一次上线时又把 F8 彻底弄坏了，原因值得记下来**：判定虽然抽成了纯函数，钩子里却还留着
一个多余的合取——

```cpp
const bool owned = !modifiers && guideIdentity && GuideHotkeyOwned(kind, guideVisible, guideFocused, focused);
```

`guideIdentity` 要求"已经有可见且有身份的攻略窗口"，而按 F8 **打开**攻略时恰恰还没有窗口，
于是 `eligible` 恒为 false：F8 被放行给游戏，什么都不发生。日志特征很好认——按下 F8 后
`guide-shortcut` **一条都没有**（连托管层都没收到），而手柄那条同时段正常。

当时的用例只测了纯函数、没测那个合取，所以全绿通过：**测试覆盖的东西和实际生效的东西不是一回事**。
现在钩子只调用这一个函数（注释里写明"外面不许再加任何 `&&`"），函数签名接收钩子实际拥有的
全部输入，`GuideHotkeyRoutingTests` 按钩子的**真实输入组合**逐条断言——其中
"`guideVisible=false, guideIdentity=false, gameFocused=true` 时 F8 仍归攻略窗口"就是这次的回归；
把旧写法放回纯函数，这条断言立刻失败。
...
- [ ] **路线暂停或已结束时**按同一个攻略键：不打开攻略，只提示「附近没有未完成点位，也没有正在导航的路线目标。」；
      点「继续指引」恢复后再按，能正常打开。
...
- [ ] 手柄 Y 在别处仍是路线菜单。
- [ ] **站在某个点旁边（它正好是路线当前目标）按攻略键**：攻略里**应该**出现可以用的「跳过」按钮
      （走到旁边才发现不好收、想下次再来，就地跳过）。
- [ ] 同一操作，但当那个点**不是**当前导航目标时：攻略里**不该**出现「跳过」。
- [ ] 打开当前目标攻略、看到「跳过」之后**暂停路线**：按钮应立刻消失；点「继续指引」再出现。
- [ ] **长按 A 收集完一个点之后不再闪避**（交还前台前先等手柄松开，见攻略浮窗文档的 2026-09-25 小节）。
- [ ] **长按 Y 跳过之后不再触发游戏动作**（同一条规则；2026-09-25 实机反馈跳过完会跳一下，已修）。
- [ ] 老配置里 G 曾绑给别的功能时，升级后**其余设置保持原样**，只有「攻略内长按跳过」显示为未设置。