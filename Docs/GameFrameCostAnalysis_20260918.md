...
## 18. DirectComposition 的两次"成立"测量（2026-09-18 14:53-14:55）——结论当晚已撤回

> **本节结论已撤回。** 原始数字没有抄错（用 `out/perf/work-isolation.csv` 重算仍然对得上），
> 但**它测的不是后来发布的那个窗口**，而且实验设计本身也支撑不起这个结论。
> 撤回的完整理由见第 22 节；下面保留当时的记录原文，便于对照。
...
**这句话已经不成立，见第 22 节。** 当时据此得到的"composition 更快"只是两个相邻时间窗的差：
这台机器的漂移（同一次运行内两个相同基准差 5.1 fps，跨运行差到 13 fps）与效应同量级，
而且 16:06 的点击修复给 composition 窗口加上了一个当时**不在场**的 `WS_EX_LAYERED`。
...
> 下面这段是 2026-09-18 14:55 写下的原话，**当晚已撤回**，保留是为了能对上第 22 节的更正。
...
**为什么撤回**：`>20 ms` 从 17.5% 到 1.8% 这个对比同样是两个相邻时间窗的差；
探针的 `none → dcomp → layered` 也是固定顺序、只跑一遍。两者都没有交替对照，
而第 17.2 节自己已经写明这台机器的漂移可达 13 fps。再加上第 22 节的窗口样式差异，
这两条测量都不足以支撑"换贴法带来 +13.6 fps"。
...
> 本节的两个数字都建立在第 18 节那个**已撤回**的对比上，见第 22 节。
...
**这个排序已作废**：那个 +13.6 fps 不成立，所以第 22 节把默认改回了分层窗口。
剩下的 5 fps 仍然是一个未归因的量，要动它得先做一次交替对照的测量。
...
> 设置 → 捕获与刷新 → **叠加层呈现方式**（2026.9.18.2 起）
> - `分层窗口（默认）` = 0
> - `DirectComposition（可选）` = 1

原文（2026.9.18.1 发布时的选项文字，**已废弃**）：
~~`分层窗口（兼容性最好）` = 0，默认 / `DirectComposition（帧率更好）` = 1~~

> **顺带修掉一个反向的下拉框。** 2026.9.18.1 的 `ComboBox_PresentMethod` 把两个选项
> 按 `DirectComposition` 在前排列，而 `SelectedIndex` 是**直接当成配置值**存下来的
> （`SelectedIndex = OverlayPresentMode`，`ConfigureAsync(overlayPresentMode: SelectedIndex)`）。
> 于是界面上显示的选项和实际生效的呈现方式是**反的**：默认值 1（composition）显示成
> 「分层窗口」，选中「DirectComposition」存进去的是 0（分层窗口）。
> 这一版把字符串顺序改成与取值一致（0 在前），并同时改了默认值，所以两者现在都对得上。
...
### 20.1 关于 schema 版本（2026.9.18.2 更正）

这一节原来写的是"**故意不迁移、不升版本**"，前提是"默认值就是既有安装一直在用的 colorkey 窗口"。
**2026.9.18.1 把这个前提去掉了**（默认改成 composition，并且用 schema 3 把旧配置也迁了过去），
所以现在必须再升一次版本把它改回来：schema **4** 把 `OverlayPresentMode` 迁回 0，
理由见第 22 节。`CaptureWay` 的 schema 2 迁移不变。
...
- **没有改默认**，也**没有走发布流程**——等玩家本地试玩验证；
  （这一条只在写下它的那一刻成立：第二天默认还是被切过去了，见第 22 节。）
...
## 22. 撤回第 18 节，并把默认改回 WGC + 分层窗口（2026-09-18 晚）

### 22.1 玩家反过来测到的结果

把 composition 设为默认（2026.9.18.1）之后，本机复测得到的**方向是反的**：
composition 比分层窗口**更低**，与第 18 节相反。

### 22.2 先排除"数字记错了"

用原始 PresentMon 记录 `out/perf/work-isolation.csv`（59,544 帧）按 10 秒重算：

| 时间段 | fps | `>20 ms` |
| --- | --- | --- |
| 14:53:20–14:54:30（分层窗口段） | 101–102 | 15.6–18.6% |
| 14:55:00–14:55:50（composition 段） | 110.7 → 116.3 | 4.0% → 1.0% |

与第 18 节的 101.3 / 114.9 对得上。**数字本身没有记错。**

### 22.3 真正的原因一：测的那个窗口不是发布的那个窗口

| 时间 | 事件 |
| --- | --- |
| 14:53–14:55 | 测量。当时 composition 窗口样式 = `TOPMOST \| TRANSPARENT \| NOACTIVATE \| TOOLWINDOW \| **NOREDIRECTIONBITMAP**`，**没有 `WS_EX_LAYERED`** |
| 16:06（`5f2bc9d`） | 修「composition 下点击无效」时把 **`WS_EX_LAYERED` 加进了两条路径** |
| 发布（2026.9.18.1） | composition 窗口 = `LAYERED \| NOREDIRECTIONBITMAP` |

`WS_EX_LAYERED` 正是改动前那份代码**特意避开**的标志，它的注释写着：
"合成视觉不需要重定向位图，而**要它会让 DWM 对这个表面走更慢的路径**"。
第 21 节的修复为了拿到点击穿透，把这个标志加了回去，**但没有重新测帧率**。

探针也一样：`IMaoOverlayPresentProbe` 的 `dcomp` 模式也是"`NOREDIRECTIONBITMAP`、不加
`LAYERED`"，即改动前的样式。**所以现在发布的这个组合，从来没有被测过。**

### 22.4 真正的原因二：实验设计本身就不成立

即使没有那个样式改动，第 18 节的结论也偏弱：

- 两个条件是**先后相接的时间窗**，不是交替对照；
- 同一份 CSV 里 composition 段自己就在爬升（110.7 → 116.3），
  而它前面"没开工具"的那段整段停在 **82 fps**——这台机器的绝对帧率在几分钟内能移动十几帧；
- 同一次运行里两个**完全相同**的基准是 **101.3 与 106.4（差 5.1 fps）**，跨运行差到 13 fps；
- 而声称的效应是 **+13.6 fps——与噪声同量级**。

第 17.2 节已经写明"跨运行比较绝对 fps 没有意义"，第 18 节却仍然用了跨窗口的绝对 fps 差。

### 22.5 顺带发现：设置页的下拉框是反的

见第 20 节。界面上显示的选项和实际生效的呈现方式相反，所以玩家在 2026.9.18.1 里
"选 DirectComposition"实际得到的是分层窗口。已改为字符串顺序与取值一致。

### 22.6 本次改动（2026.9.18.2）

1. **默认改回 `分层窗口`**：`RuntimeConfiguration.OverlayPresentMode = 0`，
   schema 升到 **4**，把 2026.9.18.1 迁移过去的配置**迁回来**（`RuntimeConfigurationStore.Migrate`）。
2. **`画面捕获方式` 默认保持 WGC**（`CaptureWay = 1`），与 schema 2 的迁移一致，本次不动。
3. **删掉所有"composition 帧率更高"的说法**：设置页选项文字与说明、第 18/19/20 节、
   `Docs/Release-2026.9.18.1.md`、以及 GitHub 上 v2026.9.18.1 的发布说明。
   DirectComposition 保留为**可选**项，但不再作为推荐。
4. **修掉下拉框反向**：`ComboBox_PresentMethod` 改为 `分层窗口` 在前。

### 22.7 仍然不知道的事（不要当成已知）

- composition 与分层窗口**到底谁快**：目前**没有**可信数据。现有的两份（第 18 节实机、
  第 14 节探针）都是固定顺序、只跑一遍、且测的是 `WS_EX_LAYERED` 缺失的样式。
- `WS_EX_LAYERED` 对 DWM 合成路径的实际代价：**未测**。
- 要回答这两个问题，需要一次**交替**测量（`dcomp` → `dcomp+LAYERED` → `layered` → 回环，
  每段 30–45 秒，同一轮 PresentMon），比较每段差值与回环一致性，而不是两个大块。
