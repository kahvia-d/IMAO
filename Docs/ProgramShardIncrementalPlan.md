...
## 10. 收益（真实数据：干跑 + 第一次线上增量实测）

数字都是**压缩后**的下载量（用户实际付的流量）。第一版是 §4.2 的干跑估算；**2026.9.20.2 发布后做了
一次真实线上增量实测**（scratch 安装 = 真实的 2026.9.20.1 程序树，清单 = 线上 stable，下载走真实 URL），
结果见本节末尾——它修正了下表里"只改 UI"那一行的预期。

| 更新类型 | 整包 | 分片后（7a） | 节省 |
|---|---|---|---|
| 只改 UI/桥（最常见） | 717 MB | **`ui` + `core` ≈56 MB（实测，见下）** | ≈92% |
| 改地图点位/攻略 | 717 MB | `assets-map-data` **1.6 MB** | ≈99.8% |
| 改图标包 | 717 MB | `assets-map-icons` 33.6 MB | ≈95% |
| 换 .NET 运行时或 Paddle | 717 MB | `runtime` 142.2 MB | ≈80% |
| 只改瓦片包 | 717 MB | `assets-tiles` 437.0 MB | ≈39% |
| 首次安装 | 717 MB | A1：全部片合计 **704.4 MB** | 略小 |

**口径说明**：717 MB 是 `2026.9.20.1`／`.20.2` 整包（含退役基础库之前的 840 MB 已不再适用）。

**第一次线上增量实测（2026.9.20.1 → 2026.9.20.2，只改了图标）**：

| 项目 | 结果 |
|---|---|
| 线上清单 | `shards = 7`，`ui` 30.42 MB / `core` 25.73 MB |
| 客户端实际下载 | **2 片 = 56.14 MB**（`core.zip` + `ui.zip`），35.8 s，随后通过整树校验并提交 |
| 对比整包 | 717.2 MB → **省 92%** |

为什么"只改图标"也会下 `core`：`IMao-CoreHost.exe` 在两次发布里**大小完全相同而哈希不同**
（36,687,360 字节，`6df86ddf…` → `7c0a836b…`）——候选构建每次重新编译原生目标，PE/链接器元数据
（时间戳、绝对路径等）就变了。同一版本里 `IMao-WinUI.dll`、`IMao-WinUI.Core.dll`、`KuroSyncBridge.exe`、
`IMao-WinUI.deps.json` 也都是"源码没动、字节变了"，只是它们都在 `ui` 片里，不额外产生下载。

**所以现实地板是 ≈56 MB，不是 ≈30 MB**，而且 `ui` 每版必变（`build-info.json`、`launcher-build-info.json`
带版本号，`IMao-Launcher.exe` 的 `FileVersion = appVersion`，`bundled-snapshot.json` 带 snapshotId）。
...
5. ✅ **端到端（过渡版已线上发布，2026-09-20）**：按 §5.4 路线 A 发布了
   **`v2026.9.20.1` / sequence 16**——整包程序发布，把认识分片的客户端送到所有已安装副本。
   ...
   - **待实测**：客户端真正下载的字节数（已在 `2026.9.20.2` 上完成，见 §12.1 与 §10）。
6. ✅ **收尾**：`Docs/ProgramUpdates.md`、`Docs/ResourceUpdates.md`（新增「过渡版与分片版的发布差异」）
   与本文档已同步；`v2026.9.20.1`（整包过渡）与 `v2026.9.20.2`（**首个分片发布**）均已上线。

### 12.1 发布记录

| 版本 | 日期 | 路线 | 清单 | 附件 | 结果 |
|---|---|---|---|---|---|
| `2026.9.20.1` | 2026-09-20 | A（整包过渡） | sequence 16，`shards=0` | update.json + 离线包 + 程序 zip（3 个，≈1.19 GB） | 已发布 |
| `2026.9.20.2` | 2026-09-20 | **分片** | sequence 17，`shards=7` + 描述符 | 7 片 + 描述符 + update.json（9 个，≈705 MB） | 已发布 |
| 之后的常态 | — | 分片 | 只变 `ui`(+`core`) | 只传变化片 + 描述符（离线包仅在资源集变化时） | — |

`2026.9.20.2` 的两个附带变化：
- **离线包按需**：本次没有任何新资源包，`prepare` 按 B 跳过离线包，发布脚本打印
  "resource set unchanged" 并且不上传 → 相比上一版少传 472 MB。`--with-offline true` 可强制。
- **图标修复**：`IMao-WinUI.exe` / `IMao-Launcher.exe` 之前嵌的是旧占位图标（`<ApplicationIcon>` 指向
