...
$guideLog = Join-Path $guideRepo 'out\main-window-harness-build.log'
...
$guideInfo.Arguments = '/d /c call "' + $guideVcvars + '" >nul && "' + $guideMsbuild +
    '" Tests\MainWindowRuntime\MainWindowRuntime.csproj /restore /t:Build /p:Configuration=Release /p:Platform=x64 /p:NuGetAudit=false' +
    ' /m > "' + $guideLog + '" 2>&1'
