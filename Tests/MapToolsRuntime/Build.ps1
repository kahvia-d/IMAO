...
$guideLog = Join-Path $guideRepo 'out\map-tools-harness-build.log'
...
$guideInfo.Arguments = '/d /c call "' + $guideVcvars + '" >nul && "' + $guideMsbuild +
    '" Tests\MapToolsRuntime\MapToolsRuntime.csproj /restore /t:Build /p:Configuration=Release /p:Platform=x64 /p:NuGetAudit=false' +
    ' /m > "' + $guideLog + '" 2>&1'
...
try {
    ...
    if ($guideBuild.ExitCode -ne 0) { throw "Map tools harness build failed: $guideLog" }
}
