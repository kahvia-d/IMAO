...
struct Snapshot {
    ...
    int level = 0;       // -2
    /// Which way this layered map's levels run; see FloorEntry::heightDirection. Only the fallback:
    /// a level does not always follow the height (虚妄摇篮), so heightRank is preferred.
